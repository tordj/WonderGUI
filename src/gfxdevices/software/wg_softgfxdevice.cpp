...
//		_src.y += 0.5f * complexTransform[1][1] + 0.5f * complexTransform[0][1];
		...
		for (int i = 0; i < m_nClipRects; i++)
		{
			...
			// Check if source clipping is needed
			...
			{
				CoordF src2 = src;
				src2.x += (patch.w - 1) * complexTransform[0][0] + (patch.h - 1) * complexTransform[1][0];
				src2.y += (patch.w - 1) * complexTransform[0][1] + (patch.h - 1) * complexTransform[1][1];

				if (src2.x < 0 || src2.y < 0 || src2.x > clipMax.w || src2.y > clipMax.h )
					bClipSource = true;
			}

			(this->*m_pComplexBlitOp)(patch, src, complexTransform,true);
		}
	}
