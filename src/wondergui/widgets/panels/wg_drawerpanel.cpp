...
namespace wg
{
	...
	void DrawerPanel::_receive(Msg * pMsg)
	{
	}
	...
	std::tuple<SizeSPX,SizeSPX> DrawerPanel::_calcDefaultSize(int scale) const
	{
		...
		if( !slots[1].isEmpty() )
		{
			...
			if( _isVertical() )
			{
				defaultOpen.w = defaultClosed.w = std::max(defaultClosed.w, def.w);
				defaultOpen.h += def.h;
			}
			else
			{
				defaultOpen.h = defaultClosed.h = std::max(defaultClosed.h, def.h);
				defaultOpen.w += def.w;
			}
		}
	...
	bool DrawerPanel::_updateGeo()
	{
		...
		switch( m_direction )
		{
			case Direction::Up:
				...
				m_backWindow = { content.x, content.y, content.w, content.h - frontSize.h };
			...
			case Direction::Down:
				...
				m_backWindow = { content.x, content.y + frontSize.h, content.w, content.h - frontSize.h };
			...
			case Direction::Left:
				...
				m_backWindow = { content.x, content.y, content.w - frontSize.w, content.h };
			...
			case Direction::Right:
				m_frontGeo = { content.x, content.y, frontSize.w, frontSize.h };
				m_backWindow = { content.x + frontSize.w, content.y, content.w - frontSize.w, content.h };
