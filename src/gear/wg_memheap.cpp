...
namespace wg
{
  ...
  void * MemHeap::malloc( int bytes )
  {
    ...
    if( !pHead )
	{
		GearBase::handleError(ErrorSeverity::Serious, ErrorCode::ResourceExhausted, "Failed to allocate memory. This MemHeap can not provide a continous area of that many bytes.", nullptr, nullptr, __func__, __FILE__, __LINE__);
		return nullptr;
	}
	...
	if( m_debugLevel > 1 )
	{
		...
		{
			GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Reserved memory outside buffer", nullptr, nullptr, __func__, __FILE__, __LINE__);
		}
  ...
  void MemHeap::free( void * pBuffer )
  {
	  ...
	  if( m_debugLevel > 0 )
	  {
		  ...
		  {
			  GearBase::handleError(ErrorSeverity::Critical, ErrorCode::InvalidParam, "Pointer to memory to free is outside bounds and therefre do not point to memory allocated by this MemHeap object.", nullptr, nullptr, __func__, __FILE__, __LINE__);
		  }
		  
		  if( pHead->boundsGuard != 0xDEADBEEF )
		  {
			  GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Header of block being freed is corrupt. Either provided pointer is wrong or we an out-of-bounds write somewhere.", nullptr, nullptr, __func__, __FILE__, __LINE__);
		  }

		  if( pHead->pNextBlock->boundsGuard != 0xDEADBEEF )
		  {
			  GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Bytes immediately following block being freed have been overwritten. Have we written too far?", nullptr, nullptr, __func__, __FILE__, __LINE__);
		  }
...
void MemHeap::_checkIntegrity()
{
	...
	for( auto pBlock = m_pFirstBlock ; pBlock != nullptr ; pBlock = pBlock->pNextBlock )
	{
		...
		{
			GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Memory Block outside bounds", nullptr, nullptr, __func__, __FILE__, __LINE__);
		}
		
		if( pBlock->boundsGuard != 0xDEADBEEF )
		{
			GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Write outside allocated memory detected", nullptr, nullptr, __func__, __FILE__, __LINE__);
		}
		
		if( pBlock->pNextBlock < &pBlock[1] && pBlock->pNextBlock != nullptr )
		{
			GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Header::pNextBlock has illegal value", nullptr, nullptr, __func__, __FILE__, __LINE__);
		}
		
		if( pBlock->pNextBlock && pBlock->pNextBlock->pPrevBlock != pBlock )
		{
			GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Block chain broken. Next block does not point back at us", nullptr, nullptr, __func__, __FILE__, __LINE__);
		}
		
		if( pBlock->pPrevBlock && pBlock->pPrevBlock->pNextBlock != pBlock )
		{
			GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Block chain broken. Prev block does not point to us", nullptr, nullptr, __func__, __FILE__, __LINE__);
		}
		
		if( pBlock->pNextFree && pBlock->pNextFree->pPrevFree != pBlock )
		{
			GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Block chain for free blocks broken. Next block does not point back to us", nullptr, nullptr, __func__, __FILE__, __LINE__);
		}

		if( pBlock->pPrevFree && pBlock->pPrevFree->pNextFree != pBlock )
		{
			GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Block chain for free blocks broken. Prev block does not point to us", nullptr, nullptr, __func__, __FILE__, __LINE__);
		}
		
		if( pBlock->pNextFree && !pBlock->pPrevFree && pBlock != m_pFirstFree )
		{
			GearBase::handleError(ErrorSeverity::Critical, ErrorCode::Internal, "Block chain for free blocks broken. Free block not really part of the chain", nullptr, nullptr, __func__, __FILE__, __LINE__);
		}
	}
