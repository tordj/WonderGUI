...
namespace wg 
{
	...
	uint32_t Surface::colorToPixel( const Color& col ) const
	{
		...
		{
			pix = ((col.r >> m_pixelFormat.R_loss) << m_pixelFormat.R_shift) |
				((col.g >> m_pixelFormat.G_loss) << m_pixelFormat.G_shift) |
				((col.b >> m_pixelFormat.B_loss) << m_pixelFormat.B_shift) |
				((col.a >> m_pixelFormat.A_loss) << m_pixelFormat.A_shift);
		}
		return pix;
	}
	...
	Color Surface::pixelToColor( uint32_t pixel ) const
	{
		...
		{
			Color col(((pixel & m_pixelFormat.R_mask) >> m_pixelFormat.R_shift) << m_pixelFormat.R_loss,
				((pixel & m_pixelFormat.G_mask) >> m_pixelFormat.G_shift) << m_pixelFormat.G_loss,
				((pixel & m_pixelFormat.B_mask) >> m_pixelFormat.B_shift) << m_pixelFormat.B_loss,
				255);
		}
		else
		{
			Color col(((pixel & m_pixelFormat.R_mask) >> m_pixelFormat.R_shift) << m_pixelFormat.R_loss,
				((pixel & m_pixelFormat.G_mask) >> m_pixelFormat.G_shift) << m_pixelFormat.G_loss,
				((pixel & m_pixelFormat.B_mask) >> m_pixelFormat.B_shift) << m_pixelFormat.B_loss,
				((pixel & m_pixelFormat.A_mask) >> m_pixelFormat.A_shift) << m_pixelFormat.A_loss);
