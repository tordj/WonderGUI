...
Waveform_p createDonut(const SizeSPX size, spx thickness, HiColor fillColor, spx outlineThickness, HiColor outlineColor, WaveformFactory * pFactory )
{
	...
	if( !pFactory )
		pFactory = GfxBase::defaultWaveformFactory();
		
	auto pWaveform = pFactory->createWaveform( WGBP(Waveform,
													_.size = outerRect.size(),
													_.segments = 9,
													_.colors = col) );
	

	pWaveform->importSamples(WaveOrigo::Top, pBuffer, 0, 4, 0, samplePoints, 1, 4);
	pWaveform->importSamples(WaveOrigo::Top, pBuffer+samplePoints * 4, 4, 8, 0, samplePoints, 1, 4);

	pWaveform->setRenderSegments(9);
...
{
	if( pWaveform->segments() < 9 )
		return false;
	
	// Center and corners in 26.6 format.

	SizeI size = pWaveform->pixelSize();
	...
	// Import generated samples to waveform
