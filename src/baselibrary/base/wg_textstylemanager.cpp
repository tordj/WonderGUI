...
namespace wg
{
	...
	TextStyle_p TextStyleManager::getPointer( TextStyle_h handle )
	{
		...
		{
			Base::handleError(ErrorLevel::Warning, ErrorCode::SystemIntegrity, "TextStyle handle out of bounds. TextStyle does not exist. Returning nullptr.", nullptr, &TYPEINFO, __func__, __FILE__, __LINE__);
			return nullptr;
		}
	...
	void TextStyleManager::exit()
	{
		if (s_size > 0)
		{
			Base::handleError(ErrorLevel::Warning, ErrorCode::SystemIntegrity, "Exiting TextStyleManager before all TextStyles have been deleted. This can cause crashes later on.", nullptr, &TYPEINFO, __func__, __FILE__, __LINE__);
			return;
		}
	...
	uint16_t TextStyleManager::_reserveHandle(TextStyle * p)
	{
		if( s_size == s_capacity )
		{
			...
			{
				Base::handleError(ErrorLevel::Critical, ErrorCode::ResourceExhausted, "Maximum number of TextStyles already created. Can have no more than 65535. Will return 0.", nullptr, &TYPEINFO, __func__, __FILE__, __LINE__);
				return 0;
			}
	...
	void TextStyleManager::_releaseHandle( TextStyle_h handle )
	{
        if( s_pLookupTable == nullptr )
        {
			Base::handleError(ErrorLevel::Warning, ErrorCode::SystemIntegrity, "TextStyle destroyed after TextStyleManager has exited (or possibly before being initialized).", nullptr, &TYPEINFO, __func__, __FILE__, __LINE__);
            return;
        }

		if (handle == 0 || handle >= s_capacity)
		{
			Base::handleError(ErrorLevel::Warning, ErrorCode::SystemIntegrity, "Attempt to release invalid TextStyle-handle. Something has gone wrong.", nullptr, &TYPEINFO, __func__, __FILE__, __LINE__);
			return;
		}
