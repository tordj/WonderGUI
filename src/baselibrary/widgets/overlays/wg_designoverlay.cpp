...
namespace wg
{
	...
	RectSPX DesignOverlay::_selectionGeo() const
	{
		...
		if (m_pSelectionSkin)
			selectedGeo += m_pSelectionSkin->_contentPadding(m_scale, State::Normal);
	...
	void DesignOverlay::_selectWidget(Widget * pWidget)
	{
		...
		if (m_pSelectionSkin)
		{
			if (m_pSelectedWidget)
			{
				...
				geo += m_pSelectionSkin->_contentPadding(m_scale, State::Normal);
				_requestRender(geo);
			}
			...
			case MsgType::MousePress:
			{
				if (_pMsg->originalSourceRawPtr() == this)			// Make sure it wasn't a propagated press on widget in palette.
				{
					...
					if (pMsg->button() == MouseButton::Left)
					{
						...
						if (m_pToolboxSkin)
						{
							BorderSPX	contentPadding = m_pToolboxSkin->_contentPadding(m_scale, State::Normal);
