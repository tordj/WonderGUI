...
public:
	...
	{
		m_pStdSurface = FileUtil::loadSurface("../resources/mipmaptest.png", pDevice->surfaceFactory());
		if (!m_pStdSurface)
			return false;
		m_pStdSurface->setScaleMode(ScaleMode::Interpolate);

		m_pMipSurface = FileUtil::loadSurface("../resources/mipmaptest.png", pDevice->surfaceFactory(), SurfaceFlag::Mipmapped );
		if (!m_pMipSurface)
			return false;
