...
//____ WgGizmoProfiler() _________________________________________________________________
...
*/
	...
	pDevice->ClipFillRect( _clip, ScreenGeometry(), WgColor(0x40,0x40,0x40,0x80) );
	...
	while( fLine < fTimeScale )
	{
		...
		WgTextTool::readString( (const char *&)pTemp, temp2, 39 );
		int x = _window.x + int(Width()*fLine/fTimeScale);
		pDevice->clipPrintLine( _clip, m_pProp, WG_MODE_NORMAL, x, x, _window.y + height*n, temp2 );
...
{
	WgGizmoProfiler * pClone		= (WgGizmoProfiler *) _pClone;

	pClone->m_pProp		= m_pProp;
}


//____ OnMarkTest() ___________________________________________________

bool WgGizmoProfiler::OnMarkTest( int _x, int _y )
{
	return false;
}
