...
#include <wg3_glsurfacefactory.h>
#include <wg3_util.h>
...
namespace wg
{
	...
	Surface_p GlSurfaceFactory::createSurface( SizeI size, PixelFormat format, int flags, const Color8 * pClut ) const
	{
		return GlSurface::create(size,format,flags | m_flags,pClut);
	}


	Surface_p GlSurfaceFactory::createSurface( SizeI size, PixelFormat format, Blob * pBlob, int pitch, int flags, const Color8 * pClut ) const
	{
		return GlSurface::create(size, format, pBlob, pitch, flags | m_flags, pClut);
	}

	Surface_p GlSurfaceFactory::createSurface( SizeI size, PixelFormat format, uint8_t * pPixels, int pitch, const PixelDescription * pPixelDescription, int flags, const Color8 * pClut ) const
	{
